...
const GDriveService = {
    ...
    async refreshAccessToken() {
        ...
        try {
            ...
            // В статическом приложении мы вызываем попап.
            const newToken = await window.refreshGoogleToken();
            return newToken;
        } catch (err) {
            console.error('Ошибка автоматического обновления:', err);
            // Вместо того чтобы просто кидать ошибку, открываем наше специальное окно переподключения
            if (window.openGDriveAuthModal) {
                window.openGDriveAuthModal();
            }
            throw new Error('Требуется подключение Google Drive. Используйте появившееся окно.');
        }
    },
